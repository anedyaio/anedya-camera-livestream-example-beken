...
/* Tracks link state so other modules can avoid pointless work while the
 * network is down — the MQTT reconnect loop checks this rather than
 * repeatedly failing DNS lookups against a dead interface. */
...
static void anedya_cam_wifi_event_cb(void *new_evt)
{
...
int anedya_cam_wifi_sta_connect(char *ssid, char *key)
{
    ...
    bk_wlan_status_register_cb(anedya_cam_wifi_event_cb);
...
int anedya_cam_socket_sendto(int *fd, const struct sockaddr *dst, uint8_t *data, uint32_t length, int offset)
{
    ...
    int max_retry = ANEDYA_CAM_SEND_MAX_RETRY;
    uint16_t index = 0;
    ...
    do
    {
        ...
        rtos_delay_milliseconds(ANEDYA_CAM_SEND_MAX_DELAY);
