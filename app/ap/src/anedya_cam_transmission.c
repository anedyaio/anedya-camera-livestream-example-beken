...
{
    struct timeval tv;
} anedya_cam_trans_info_t;

anedya_cam_trans_info_t g_anedya_cam_trans_info = {0};
/*******************************************************************************
* crc8
*
...
void anedya_cam_hex_dump(uint8_t *data, uint32_t length)
{
#ifdef DUMP_DEBUG
...
db_channel_t *anedya_cam_transmission_malloc(uint16_t max_rx_size, uint16_t max_tx_size)
{
...
void anedya_cam_transmission_unpack(db_channel_t *channel, uint8_t *data, uint32_t length, anedya_cam_transmission_recive_cb_t cb)
{
    ...
    anedya_cam_hex_dump(data, length);
#else
    LOGV("recv unpack: %u\n", length);
#endif
    ...
    while (left != 0)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                anedya_cam_hex_dump(channel->cbuf, channel->ccount);
                //TODO FIXME
                break;
            }
...
uint32_t anedya_cam_transmission_get_milliseconds(void)
{
    uint32_t time = 0;
    ...
    bk_rtc_gettimeofday(&(g_anedya_cam_trans_info.tv), 0);

    long time_offset = 8 * 3600; //8 hour offset
    long ms_time = (g_anedya_cam_trans_info.tv.tv_usec / 1000);
    long time_ms = 0;

    if (g_anedya_cam_trans_info.tv.tv_sec < time_offset)
    {
        //LOGD(" local time sec: %ld \r\n",g_anedya_cam_trans_info.tv.tv_sec);
        time_ms = (g_anedya_cam_trans_info.tv.tv_sec * 1000) + ms_time;
    }
    else
    {
        time_ms = ((g_anedya_cam_trans_info.tv.tv_sec - time_offset) * 1000) + ms_time;
    }

    time = (time_ms & 0xFFFFFFFF);

    //LOGD("sec: %ld time %d\r\n", g_anedya_cam_trans_info.tv.tv_sec ,time);
#endif

    return time;
}

void anedya_cam_transmission_pack(db_channel_t *channel, uint8_t *data, uint32_t length)
{
    ...
    */
    ...
    head->timestamp = CHECK_ENDIAN_UINT32(anedya_cam_transmission_get_milliseconds());
...
int anedya_cam_transmission_pack_send(db_channel_t *channel, uint8_t *data, uint32_t length, anedya_cam_transmission_send_t cb)
{
    bk_err_t ret = BK_FAIL;

    anedya_cam_transmission_pack(channel, data, length);
