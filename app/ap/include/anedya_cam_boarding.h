...
typedef struct
{
	...
	uint16_t channel;
} anedya_cam_boarding_info_t;


int anedya_cam_boarding_init(void);
void anedya_cam_boarding_event_notify(uint16_t opcode, int status);
void anedya_cam_boarding_ble_disable(void);
int anedya_cam_boarding_notify(uint8_t *data, uint16_t length);
